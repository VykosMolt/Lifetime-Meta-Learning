"""
O1 oracle-reachability gate — executable analysis, v1.5.3.
...
def budget_mde(
    ...
    """Smallest delta detectable at target_power under a budget-constrained G."""
    k_crit = mcnemar_critical_table(G, alpha_level)
    lo, hi = 0.0, q_disc
    while hi - lo > tol:
        mid = (lo + hi) / 2
...
MANIFEST_REQUIRED = (
    ...
    "action_space.writable_boundary.layer_index_zero_based",
    ...
    "power.G_power", "power.G_max", "power.G_confirm",
...
def gate_G17_manifest_consistency(manifest: Manifest) -> None:
    ...
    if gc != want_gc:
        raise IntegrityError(
            "G17_MANIFEST_CONSISTENCY",
            f"G_confirm {gc} != min(G_power {gp}, G_max {gm}) = {want_gc}")
    expected_verdict = "PROCEED_POWERED" if gp <= gm else "PROCEED_BUDGET_CONSTRAINED"
    if verdict != expected_verdict:
        raise IntegrityError(
            "G17_MANIFEST_CONSISTENCY",
            f"calibration verdict {verdict} inconsistent with G_power={gp}, G_max={gm}; "
...
def run_primary(
    ...
    verify_pregeneration_seal(
        ...
        calibration_results_path, axis_package_path,
    )
    ...
    cal = load_task_manifest(calibration_task_manifest_path)
