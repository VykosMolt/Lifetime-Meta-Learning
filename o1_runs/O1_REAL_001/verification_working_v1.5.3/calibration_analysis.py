"""
Executable calibration derivation for O1, v1.5.3.
...
CALIBRATION_CONFIG_PATHS = (
    ...
    "model.checkpoint_sha256", "model.loops", "model.dtype",
...
def _difficulty_assignment(tasks: dict[str, dict], bands: dict[str, list[float]]) -> tuple[dict[str, list[str]], dict[str, dict]]:
    ...
    for setting, rows in sorted(settings.items()):
        ...
        for stratum in ("hard_medium", "medium"):
            ...
            upper_ok = p <= hi if stratum == "medium" else p < hi
            if p >= lo and upper_ok:
...
def _coherence_for_alpha(tasks: dict[str, dict], tids: list[str], setting_to_stratum: dict[str, str], alpha: float,
    ...
    for tid in tids:
        ...
        if any(len(r["generated_token_ids"]) == 0 for r in rows):
...
def compute_calibration(records_path: str, metadata_path: str, manifest_path: str,
                        task_manifest_path: str, precommit_path: str) -> dict:
    ...
    if calibration_verdict == "PENDING_POWER":
        if q_upper < delta:
            ...
            if G_power == -1:
                ...
                G_confirm = G_max
                mde = budget_mde(G_max, q_upper, target_power, alpha_level) if G_max > 0 else q_upper
            else:
                chosen = None
                for plan in unique_plans:
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = compute_calibration(ns.records, ns.metadata, ns.manifest, ns.task_manifest, ns.precommit)
