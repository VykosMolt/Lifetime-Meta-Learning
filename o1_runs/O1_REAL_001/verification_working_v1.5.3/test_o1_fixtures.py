"""
Synthetic fixtures for the O1 v1.5 gates.
...
def _row(task, arm, stream, seed, wf, vc, alpha, d=None, s=None, toks=None,
    ...
    return {"task_id": task, "stratum": "medium" if int(task.split("_")[1]) % 2
            else "hard_medium", "arm": arm, "alpha": alpha, "direction_id": d,
            "sign": s, "stream_index": stream, "seed": seed, "well_formed": wf,
            "verifier_correct": vc, "generated_token_ids": toks,
            "n_generated_tokens": len(toks), "hit_max_tokens": False,
    ...
    for g in range(G_cal):
        ...
        for i in range(8):
            rows.append({"task_id": task,
                         ...
                         "arm": "baseline", "alpha": 0.0,
                         "direction_id": None, "sign": None,
                         "stream_index": i, "seed": seeds[i],
                         "well_formed": True,
                         "verifier_correct": bool(base_ok and i == 0),
                         "generated_token_ids": [10 + g, 20 + i],
                         "logits_finite": True,
        ...
        for a in grid:
            for k, (d, sg) in enumerate(DIRS):
                ...
                rows.append({"task_id": task,
                             "task_content_sha256": f"cal_content_{g}",
                             "generator_setting_id": setting,
                             "arm": "structured", "alpha": a,
                             "direction_id": d, "sign": sg,
                             "stream_index": k, "seed": seeds[k],
                             "well_formed": True,
                             "verifier_correct": bool(int_ok and k == 0),
                             "generated_token_ids": [10 + g, 30 + k],
                             "logits_finite": True,
                             "catastrophic_repetition": bool(a in (0.04, 0.08) and k == 0),
                             "calibration_precommit_sha256": precommit_hash})
...
def write_run(recs, G=None, arms=ARMS, tag="run", **overrides):
    """Write a complete v1.5 synthetic run, including calibration and preseal."""
    ...
    conf_p, cal_p, seed_p = (os.path.join(d, n) for n in
    ...
    man = {
        "prereg_version": "1.5", "sealed_at_utc": "2026-07-28T00:00:00Z",
        ...
        "model": {"checkpoint_sha256": sha256_tree(checkpoint_dir), "loops": 4,
                  "dtype": "bfloat16",
    ...
    cal_result = compute_calibration(cal_rec, cal_meta, man_p, cal_p, cal_precommit)
...
def test_15_completed_subset_changes_G():
    ...
    # truncate the task manifest too, so only the sealed G still disagrees
    run2 = write_run(subset, G=20, tag="f15b")
    e = expect("G12_EXACT_G", V, subset, run2)
...
def test_21_zero_alpha_symmetric_divergence_halts():
    ...
    for r in recs:
        if r["arm"] == "zero_alpha":
            r["generated_token_ids"] = [777] + r["generated_token_ids"][1:]
...
def test_47_zero_alpha_metadata_must_be_exactly_identical():
    ...
    # same derived R, different parser/verifier state
    recs = make_clean(G=40, n_base=lambda g: 0)
    for r in recs:
        if r["task_id"] == "conf_0000" and r["stream_index"] == 0:
            if r["arm"] == "baseline":
                r["well_formed"], r["verifier_correct"] = True, False
            if r["arm"] == "zero_alpha":
                r["well_formed"], r["verifier_correct"] = False, True
...
def test_48_provenance_must_resolve_the_full_configuration():
    ...
    prov = json.load(open(run["prov"]))
    del prov["resolved_generation_config"]["action_space"]["writable_boundary"]["layer_index_zero_based"]
    json.dump(prov, open(run["prov"], "w"))
    # Recommit the tampered provenance so the pre-generation gate does not mask G14.
    os.remove(run["pregen"])
    create_pregeneration_seal(run["pregen"], run["manifest"], run["prov"],
...
def test_49_generation_at_the_wrong_boundary_caught():
    ...
    prov = json.load(open(run["prov"]))
    prov["resolved_generation_config"]["action_space"]["writable_boundary"]["layer_index_zero_based"] = 47
