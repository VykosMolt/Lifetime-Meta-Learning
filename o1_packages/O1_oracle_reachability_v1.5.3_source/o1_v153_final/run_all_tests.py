"""Run every shipped O1 v1.5.3 verification suite and static package check."""
...
SUITES = [
    ...
    ("v1.5.3 integration fixtures", [sys.executable, "test_v15_integrity.py"], "13/13 passed"),
...
def main() -> int:
    print("O1 v1.5.3 full verification", flush=True)
    print("=" * 72, flush=True)
    ...
    assert manifest["prereg_version"] == "1.5"
    ...
    for name in ("README.md", "PREREGISTRATION.md", "FREEZE_MANIFEST.template.json"):
        ...
        if ("Preregistration v1.4" in text or "50/50 passed" in text or
                "52/52" in text or "v1.5.2" in text or "11/11" in text):
    ...
    print("=" * 72)
    print("ALL O1 v1.5.3 CHECKS PASSED")
    return 0
