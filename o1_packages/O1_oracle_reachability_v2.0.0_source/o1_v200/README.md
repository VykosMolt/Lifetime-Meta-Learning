# O1 oracle-reachability preregistration package — v2.0.0
...
5. Run calibration and derive results mechanically.
6. Build the final manifest, seed matrix, provenance, and
...
The shipped runtime is `run_o1_v2_generation.py`. It binds physical layer 24
to `model.model.layers[23]`, applies the edit only at zero-based loop 2 during
prefill, preserves a true identity path at alpha zero, and exposes the
normalized L4_47 pre-decode boundary for paired transport measurement. Frozen
tasks are built by `build_o1_v2_cohorts.py`; the calibration seed matrix uses
`build_seed_matrix.py --calibration`.
