"""Run every shipped O1 v2.0 verification suite and static package check."""
...
SUITES = [
    ...
    ("calibration fixtures", [sys.executable, "test_calibration_fixtures.py"], "17/17 passed"),
    ("v2.0 integration fixtures", [sys.executable, "test_v20_integrity.py"], "15/15 passed"),
...
def main() -> int:
    print("O1 v2.0 full verification", flush=True)
    print("=" * 72, flush=True)
    ...
    assert manifest["prereg_version"] == "2.0"
    ...
    for name in ("README.md", "PREREGISTRATION.md", "FREEZE_MANIFEST.template.json"):
        ...
        if ("Preregistration v1.4" in text or "50/50 passed" in text or
                "52/52" in text or "v1.5.2" in text or "11/11" in text or
                "d4_components_l3_24.npy" in text or
                "d2_empirical_outcome" in text):
    ...
    print("=" * 72)
    print("ALL O1 v2.0 CHECKS PASSED")
    return 0
