...
## 14. v2.1 pre-calibration amendments

All amendments below were designed, implemented, fixture-tested and sealed
BEFORE any real O1 calibration or confirmatory outcome existed. They complete
or repair the v2.0 executable design without touching the scientific design:
the bank, endpoint, K=8, CRN geometry, alpha grid and selection rule,
difficulty-band values, delta target, power machinery, populations, and drop
order are unchanged. The v2.0.0 calibration precommit is superseded; a new
precommit against this package must be externally committed before
calibration.

1. **Strict commitment grammar.** The v2.0 parser accepted any A/B/C-initial
   word after the marker ("FINAL ANSWER: Correct" parsed as C), letting
   degenerate text become 1/3-chance lottery tickets inside the max-of-8
   endpoint. v2.1 accepts only a complete answer line (bare or
   angle-bracketed letter, at most one trailing period). Adversarial parser
   fixtures are part of the shipped suite.

2. **Sealed generation-to-record bridge.** All arm dispatch, Latin-square
   stream assignment, seed lookup, record assembly, and transport
   computation now live in the sealed `run_o1_v2_orchestrator.py` /
   `o1_transport_v2.py`, both hash-bound in the manifest and precommit. No
   operator-side code can produce an analysis-valid row.

3. **Recomputed scoring.** Records carry `generated_text` plus text/token
   digests. Gates G22 (confirmatory) and C10 (calibration) recompute
   `well_formed`, `verifier_correct`, `R`, and the mechanical repetition
   flag from the stored text/tokens with the sealed parser and verifier and
   reject stored metadata that disagrees.

4. **Pool-capacity closure.** The sealed candidate pool is precommit-bound
   (`cohorts.confirmatory_candidate_pool_sha256`). Calibration derives
   `G_pool` (eligible banded capacity) and the full deterministic
   confirmatory allocation (50/50 stratum target, odd remainder to
   hard_medium, supply waterfall, per-setting round robin over
   lexicographic order, hash-ordered prefixes, deterministic stratum
   ranks). `G_confirm = min(G_power, G_max, G_pool)`; pool exhaustion is the
   declared `POOL_CAPACITY_CONSTRAINED` state, never a later deadlock.
   `build_confirmatory_cohort.py` materializes the cohort with zero operator
   discretion, and gate G23 requires every confirmatory task to be a
   byte-identical pool member.

5. **Band edges.** Both difficulty bands are half-open `[lo, hi)` in code,
   as the sealed design always stated; a setting at exactly 0.70 is in
   neither band. Boundary fixtures at 0.40 / 0.55 / 0.70 are shipped.

6. **Honest constrained sensitivity.** `budget_mde` returns an explicit
   no-detectable-effect state (`null` + `mde_status`) when no delta up to
   `q_disc` reaches target power at the available G, instead of reporting
   `q_disc` as a sensitivity.

7. **Real-schema calibration manifest loader.** The confirmatory pipeline
   loads the outcome-blind calibration cohort (placeholder stratum,
   per-setting ranks) through a dedicated loader; v2.0 would have rejected
   the real sealed cohort at confirmatory analysis time.

8. **Runtime hardening.** Deterministic environment flags are strict-set,
   and torch/numpy/transformers exact versions are precommit-bound and
   runtime-asserted by the orchestrator before generation.

9. **Red-team amendments (same pre-calibration window).** An adversarial
   attack pass against the amendments above found four further defects, all
   fixed with regression fixtures before sealing:
   (a) the strict commitment regex had two adjacent quantifiers over the same
   whitespace class and backtracked quadratically, so one oversized
   `generated_text` could stall the sealed scoring gates; the tail is now an
   atomic group accepting the identical language, and the record schema caps
   `generated_text` length and binds `n_generated_tokens` to the sealed
   decoding budget.
   (b) `generated_token_ids` were bound only to their own digest, while the
   mechanical repetition flag and the empty-continuation criterion — both
   coherence inputs that select `alpha_star` — were derived from them; forged
   tokens carrying byte-identical text could therefore move the sealed
   intervention magnitude. The empty-continuation criterion is now text-bound,
   the repetition flag is recomputed by the orchestrator from the token ids it
   generated, and the sealed orchestrator re-decodes every row (including rows
   carried in from an earlier session) with the sealed tokenizer and requires
   equality with the stored text before calibration metadata is written.
   `verify_record_binding.py` re-runs that check independently at any later
   time.
   (c) resume trusted the per-row binding hash alone, so a pre-populated
   records file could pass through untouched; resumed rows are now re-scored
   under the sealed parser and verifier.
   (d) the derived boundary cache was a trust root for transport measurement;
   it is now verified against a same-session recomputation (with exact token
   reproduction required on the resume path) and never accepted on its own.
   Two further findings were closed by construction: measured throughput is
   bounded by a sealed plausibility ceiling and a zero-capacity outcome is the
   declared `NO_TIME_CAPACITY` halt rather than a proceed verdict with an
   unallocatable cohort.

   Residual, documented, and not further mitigable in code:
   `elapsed_wall_seconds` is an operator-side measurement with no independent
   attestation. It affects only `G_max`, arm dropping, and the
   powered-versus-constrained verdict — never `H_reach`, `alpha_star`, or the
   difficulty bands — so it can misstate the achieved power of the
   confirmatory run but cannot bias its estimate.
